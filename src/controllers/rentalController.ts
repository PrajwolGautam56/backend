...
import Rental, { RentalStatus, PaymentStatus, OrderStatus, PaymentMethod } from '../models/Rental';
...
// Generate payment records for a rental - ONLY from start date onwards
const generatePaymentRecords = async (
  ...
  try {
    ...
    // Start generating from the rental start date (not before it)
    ...
    // Only generate records from start month up to maxMonth (never before start month)
    while (paymentMonth <= maxMonth) {
      // Safety check: Skip if this month is before the rental start month (shouldn't happen, but just in case)
      ...
      if (!existingRecord) {
        ...
        let dueDate: Date;
        if (monthIndex === 0 || paymentMonth.getTime() === firstPaymentMonth.getTime()) {
...
export const getMyRentals = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Get user to match by email as well
    const user = await User.findById(req.userId);
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }

    // Find rentals by userId OR customer_email (case-insensitive)
    // This ensures we get all rentals even if userId wasn't set during checkout
...
export const updateRental = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (updateData.order_status && oldRental.order_status !== updateData.order_status) {
      // Set timestamps based on order status
      if (updateData.order_status === OrderStatus.CONFIRMED && !rental.order_confirmed_at) {
        updateData.order_confirmed_at = new Date();
      }
      if (updateData.order_status === OrderStatus.DELIVERED && !rental.delivered_at) {
        updateData.delivered_at = new Date();
      }
      
      logger.info('Order status updated', {
        rentalId: rental.rental_id,
        oldOrderStatus: oldRental.order_status,
        newOrderStatus: updateData.order_status
      });
    }
...
export const deletePaymentRecord = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const deletedPayment = rental.payment_records[paymentIndex];
    
    // Remove the payment record
    rental.payment_records.splice(paymentIndex, 1);
    await rental.save();
...
// Send reminder for specific payment record (Admin)
export const sendReminderForMonth = async (req: AuthRequest, res: Response) => {
  try {
    const { id, paymentId } = req.params;
    ...
    const paymentRecord = rental.payment_records.find(
      (p: any) => p._id.toString() === paymentId
    );
    ...
    if (daysDifference < 0) {
      ...
      sendEmailInBackground(
        'Rental overdue payment reminder',
        () => sendOverduePaymentReminder(rental.toObject(), paymentRecord.toObject(), daysOverdue),
        {
          rentalId: rental.rental_id,
          email: rental.customer_email,
          paymentId,
      ...
      // Payment is pending (due today or in future)
      sendEmailInBackground(
        'Rental payment reminder',
        () => sendPaymentReminder(rental.toObject(), paymentRecord.toObject(), daysDifference),
        {
          rentalId: rental.rental_id,
          email: rental.customer_email,
          paymentId,
    ...
    logger.info('Reminder sent for specific payment record', { 
      rentalId: rental.rental_id, 
      paymentId,
      month: paymentRecord.month,
      daysDifference
    ...
    res.status(200).json({
      ...
      data: {
        ...
        reminderType: daysDifference < 0 ? 'overdue' : 'pending'
      }
    });
  } catch (error: any) {
    logger.error('Error sending reminder for specific month:', error);
    res.status(500).json({
    ...
    res.status(200).json({
      ...
      data: {
        ...
        order_status: {
          ...
          pending_orders: orderStatusStats[OrderStatus.PENDING] || 0,
          processing_orders: orderStatusStats[OrderStatus.PROCESSING] || 0,
          confirmed_orders: orderStatusStats[OrderStatus.CONFIRMED] || 0,
          out_for_delivery: orderStatusStats[OrderStatus.OUT_FOR_DELIVERY] || 0,
          delivered_orders: orderStatusStats[OrderStatus.DELIVERED] || 0,
          cancelled_orders: orderStatusStats[OrderStatus.CANCELLED] || 0,
          refunded_orders: orderStatusStats[OrderStatus.REFUNDED] || 0
        },
  ...
  try {
    ...
    if (!order_status || !Object.values(OrderStatus).includes(order_status)) {
      return res.status(400).json({
        ...
        valid_statuses: Object.values(OrderStatus)
      });
    }
    ...
    const oldOrderStatus = rental.order_status;
    ...
    // Set timestamps based on order status
    if (order_status === OrderStatus.CONFIRMED && !rental.order_confirmed_at) {
      updateData.order_confirmed_at = new Date();
    }
    if (order_status === OrderStatus.DELIVERED && !rental.delivered_at) {
      updateData.delivered_at = new Date();
    }
    if (order_status === OrderStatus.OUT_FOR_DELIVERY) {
      // Can set delivery date when marking as out for delivery
      if (delivery_date) {
    ...
    logger.info('Order status updated', {
      rentalId: updatedRental.rental_id,
...
export const confirmOrder = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    return await updateOrderStatus({ ...req, body: { order_status: OrderStatus.CONFIRMED } } as any, res as any);
  } catch (error: any) {
...
// Quick action: Mark as out for delivery
export const markOutForDelivery = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { delivery_date } = req.body;
    return await updateOrderStatus({ ...req, body: { order_status: OrderStatus.OUT_FOR_DELIVERY, delivery_date } } as any, res as any);
  } catch (error: any) {
...
export const markAsDelivered = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    return await updateOrderStatus({ ...req, body: { order_status: OrderStatus.DELIVERED } } as any, res as any);
  } catch (error: any) {
  ...
  try {
    ...
    allRentals.forEach(rental => {
      ...
      ordersByStatus[orderStatus].push({
        ...
        customer_name: rental.customer_name,
        customer_email: rental.customer_email,
        total_amount: (rental.total_monthly_amount || 0) + (rental.total_deposit || 0) + (rental.delivery_charge || 0),
        order_placed_at: rental.order_placed_at || (rental as any).createdAt,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        total_orders: allRentals.length
      }
    });
  } catch (error: any) {
