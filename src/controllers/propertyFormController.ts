...
// Get user's own property forms
export const getMyPropertyForms = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      return res.status(401).json({ message: 'Unauthorized' });
    }

    // Find property forms by userId
    const propertyForms = await PropertyForm.find({ userId: req.userId })
      .sort({ createdAt: -1 })
      .lean();

    // Populate property details
    const propertyIds = [...new Set(propertyForms.map(p => p.property_id))];
    ...
    const formattedForms = propertyForms.map((form: any) => {
    ...
    logger.error('Error fetching user property forms:', error);
    res.status(500).json({
      message: 'Error fetching property forms',
      error: error.message
    });
  }
};
